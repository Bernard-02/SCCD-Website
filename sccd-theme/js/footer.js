...
function loadFooterLogo(container) {
  ...
  // WP hack 版：用 SCCD_THEME_URL 為基底，避免 origin/pages 子目錄解析錯誤
  const themeBase = window.SCCD_THEME_URL || (window.location.pathname.includes('/pages/') ? '/' : '');
  const logoPath = new URL(`data/${pickLogoFile(mode)}`, themeBase || window.location.origin).href;
...
export function initFooter() {
  ...
  if (footerContainer) {
    const footerUrl = new URL('pages/footer.html', window.SCCD_THEME_URL || window.location.origin).href;
