...
function restoreMobileGenerateLogo() {
  ...
  const swap = () => {
    ...
    const logoPathM = new URL(`data/${logoFileM}`, window.SCCD_THEME_URL || window.location.origin).href;
...
export function initHeader() {
  ...
  function setupHeaderLogic() {
    ...
    if (logo && typeof lottie !== 'undefined' && currentPage !== 'generate') {
      ...
      const logoPath = new URL(`data/${logoFile}`, window.SCCD_THEME_URL || window.location.origin).href;
    ...
    if (mobileLogo && typeof lottie !== 'undefined' && currentPage !== 'generate') {
      function loadMobileLogo() {
        ...
        mobileLogoAnim.addEventListener('DOMLoaded', () => {
          const svg = mobileLogo.querySelector('svg');
          if (svg) {
            svg.style.overflow = 'visible';
            svg.setAttribute('viewBox', '0 0 1080 1080');
      ...
      window.addEventListener('theme:changed', () => {
        const newMode = document.body.classList.contains('mode-color') ? 'color'
                      : document.body.classList.contains('mode-inverse') ? 'inverse' : 'standard';
  ...
  if (headerContainer) {
    const headerUrl = new URL('pages/header.html', window.SCCD_THEME_URL || window.location.origin).href;
    fetch(headerUrl)
      .then(res => {
