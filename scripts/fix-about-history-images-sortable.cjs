// about_history_images 改成「側欄點一下直接進、照片可拖曳排序」（2026-09-11；09-12 更正）：
// - ⭐ interface 保持 `files`（activities 全系列證明 files-M2M 的 files interface 就有 ⠿ 把手＋Upload File，
//   條件＝junction sort 欄+parent relation sort_field，兩者本就具備）。曾一度誤改 list-m2m→反而沒把手，已改回 files。
...
(async () => {
  ...
  // 2) images 欄 interface 保持 files（拖曳把手來自 relation sort_field，files interface 本就支援；勿改 list-m2m）
  console.log('② images 欄 interface 確保 = files...');
  await req('PATCH', '/fields/about_history_images/images', { meta: { interface: 'files' } });
