...
function buildImagesStack(project) {
  return project.images.map((img, i) => {
    return `<img src="${img}" alt="Work" class="absolute object-cover class-img class-img--${i+1}" data-z="${i+1}">`;
  }).join('');
}
...
function buildProjectHtml(project) {
  ...
  if (project.layout === 'image-left') {
    return `
      <div class="grid-12">
        <div class="col-span-12 md:col-start-2 md:col-span-5 relative works-container md:mb-0">${stack}</div>
  ...
  return `
    <div class="grid-12">
      ...
      <div class="col-span-12 md:col-start-7 md:col-span-5 relative works-container order-1 md:order-none md:mb-0">${stack}</div>
    </div>`;
}
